...
- Late-arriving RSS episodes are a delivery gate. Before final delivery and before marking seen, rerun a current-RSS audit for the report window and at least the previous closed window. If any episode now appears in RSS with a `published_at` inside an already-covered M/W/F window but is not manifested/seen, stop normal delivery, add it to the correct report window, collect transcripts, regenerate the report, and record the incident. Do not rely on the original run's cached RSS as proof of completeness.
